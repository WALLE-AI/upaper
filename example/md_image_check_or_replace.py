...
def resolve_oss_url(image_id: str) -> Optional[str]:
    ...
    key = "hf_papers/"+image_id
    public_url = oss_images_url(key)
    return public_url
...
def replace_md_image_links_auto(md_text: str,file_dir) -> Tuple[str, List[Dict[str, Any]]]:
    ...
    for match in IMG_PATTERN.finditer(md_text):
        ...
        if is_external_or_html(old_src):
            ...
            # 通过OSS拿到最终地址或HTML
            resolved = resolve_oss_url(file_dir+"/"+old_src)
...
def process_markdown_file(md_path: str) -> Dict[str, Any]:
    ...
    new_text, replacements = replace_md_image_links_auto(original_text,p.stem)
...
if __name__ == "__main__":
    # 把这里改成你的本地md文件路径
    md_path = "hf_papers/2510.19338/2510.19338_report.md"
